...
| `pf_artifact_blend_v29` | No-TabICL no-exact artifact; dynamic rerun-aware PF/artifact weight: highest component-gap well uses 75/25, others 80/20 | Replace invalid public-ID probing with a rule that acts on whichever hidden-rerun well has the largest PF/artifact disagreement | pending, ref 54011314 | CPU-safe feature-based probe; local public output selected `00e12e8b`, but the submitted rerun will select by hidden-test component gap |
