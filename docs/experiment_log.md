...
| `pf_artifact_blend_v23` | No-exact 80/20, but enable the artifact TabICL branch on Kaggle GPU | Test whether the artifact-stack component can change the residual structure rather than only changing scalar blend weights | pending | Larger-method candidate; expect longer notebook runtime because Kaggle reruns with GPU |
