...
| `pf_artifact_blend_v28` | Fixed h0.17 PF plus no-exact artifact 80/20; TabICL enabled on Kaggle CUDA image | Retry v26 with the public kojimar CUDA docker image and `NvidiaTeslaT4` shape so torch has CUDA support | pending | High-leverage GPU runtime fix; submit only if output summary confirms TabICL ran |
