...
| `pf_artifact_blend_v29` | No-TabICL no-exact artifact; dynamic rerun-aware PF/artifact weight: highest component-gap well uses 75/25, others 80/20 | Replace invalid public-ID probing with a rule that acts on whichever hidden-rerun well has the largest PF/artifact disagreement | pending | CPU-safe feature-based probe; submit if notebook completes |
