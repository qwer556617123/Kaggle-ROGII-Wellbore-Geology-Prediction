...
4. Shift to larger method changes:
   ...
   - While GPU is blocked, the only CPU-safe probe worth spending on is per-well no-exact weighting. The v22 component audit shows `00e12e8b` has the largest PF-artifact disagreement, so test `00e12e8b` at 75/25 while keeping the other wells at 80/20.
