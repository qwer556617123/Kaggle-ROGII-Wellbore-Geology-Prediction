...
4. Shift to larger method changes:
   ...
   - Version 23 is the next larger change: keep no-exact 80/20, but run the v10 artifact stack with TabICL enabled on Kaggle GPU. This is expected to be slower; use it to test whether a new component changes the error pattern, not as another scalar-weight tune.
