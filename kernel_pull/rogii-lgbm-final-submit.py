import numpy as np, pandas as pd, joblib, re, json
...
MODEL_PATH    = "/kaggle/input/datasets/qwer556617123/rogii-lgbm-model/lgbm_v23_spatial.pkl"
NBR_STATS_PATH = "/kaggle/input/datasets/qwer556617123/rogii-lgbm-model/test_well_nbr_stats.json"
...
print("Model loaded, features:", len(FCOLS), " best_iter:", model_meta["best_iter"])

# Load pre-computed neighbor stats (full 773-well pool: train+val, post-PS centroid)
with open(NBR_STATS_PATH) as f:
    nbr_stats = json.load(f)
print(f"Neighbor stats loaded for {len(nbr_stats)} test wells")
...
for well_id in well_ids:
    ...
    # Use pre-computed neighbor stats (from training pool — ensures consistency with model training)
    nbr = nbr_stats.get(well_id, {
        "nbr_alpha_x": 0.0, "nbr_alpha_y": 0.0,
        "nbr_dist_min": 9999.0, "nbr_r2_mean": 0.0,
        "nbr_count_consistent_x": 0.5,
    })
    nbr_ax = nbr["nbr_alpha_x"]
    nbr_ay = nbr["nbr_alpha_y"]

    hw_x = hw["X"].astype(float).values
    hw_y = hw["Y"].astype(float).values
    x_ps = hw_x[ps] if ps < len(hw_x) else hw_x[-1]
    y_ps = hw_y[ps] if ps < len(hw_y) else hw_y[-1]
    post_x = hw_x[df_post.index]
    post_y = hw_y[df_post.index]
    dx = post_x - x_ps
    dy = post_y - y_ps

    df_post = df_post.copy()
    df_post["nbr_alpha_x"]            = np.float32(nbr_ax)
    df_post["nbr_alpha_y"]            = np.float32(nbr_ay)
    df_post["nbr_3d_corr"]            = (nbr_ax * dx + nbr_ay * dy).astype(np.float32)
    df_post["nbr_dist_min"]           = np.float32(nbr["nbr_dist_min"])
    df_post["nbr_r2_mean"]            = np.float32(nbr["nbr_r2_mean"])
    df_post["nbr_count_consistent_x"] = np.float32(nbr["nbr_count_consistent_x"])
    ...
    corr_val = (nbr_ax * dx + nbr_ay * dy)
    print("  " + well_id + ": " + str(len(post_indices)) + " rows, TVT [" + str(round(tvt_pred[0],1)) + ", " + str(round(tvt_pred[-1],1)) + "]  nbr_3d_corr=[" + str(round(float(corr_val.min()),1)) + ", " + str(round(float(corr_val.max()),1)) + "]")
