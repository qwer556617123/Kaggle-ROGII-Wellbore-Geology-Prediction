...
FEAT_DIR  = DATA_DIR / "features"; MODELS_DIR= DATA_DIR / "models"; SUBS_DIR = DATA_DIR / "submissions"
...
def tw_gr_at_tvt(tw_tvt,tw_gr,tvt_arr):
    return np.interp(tvt_arr,tw_tvt,tw_gr,left=tw_gr[0],right=tw_gr[-1])
...
def build(hw, tw, well_id, split="train"):
    ...
    if split=="train" and "TVT" in hw.columns:
        ...
        pre_z=z[:ps]; known=tvt_inp[:ps].values
        valid=~np.isnan(known); pre_z=pre_z[valid]; pre_tvt=known[valid]
    if len(pre_z)>5:
    ...
    total_z_change_post=z_end-Z_anchor
    total_md_post=(float(post_md[-1])-float(post_md[0])) if n_post>1 else 1.
    post_dz_rate=total_z_change_post/total_md_post
    physics_tvt_at_end=slope*total_z_change_post
    pre_dz_md=((z[ps-1]-z[0])/(md[ps-1]-md[0])) if ps>1 and abs(md[ps-1]-md[0])>0.01 else -0.01
    dz_rate_change=post_dz_rate-pre_dz_md
    gr_at_physics=tw_gr_at_tvt(tw_tvt,tw_gr,anchored_physics)
    ...
    feat.update({"md":md,"z":z,"dz_dmd":np.gradient(z,md),"dx_dmd":np.gradient(x,md),
        ...
        "total_z_change_post":np.full(n,total_z_change_post),
        "physics_tvt_at_end":np.full(n,physics_tvt_at_end),
...
for i,wid in enumerate(all_well_ids):
    ...
    if(i+1)%100==0: print(f"  processed {i+1}/{len(all_well_ids)}")
...
post_cnt=(all_tr["is_post_ps"]==1).sum()
print(f"Features: {len(FCOLS)}, post-PS rows: {post_cnt}")
...
print("Training final model on all 773 wells (7983 rounds)...")
model_final=lgb.train(params,lgb_tr,num_boost_round=7983,callbacks=[lgb.log_evaluation(2000)])
joblib.dump(model_final,MODELS_DIR/"lgbm_final.pkl")
print("Saved lgbm_final.pkl")

sub=pd.read_csv(DATA_DIR/"sample_submission.csv")
for wid in TEST_WELLS:
    hw,tw=load_well(wid,"test"); ps=get_ps(hw)
    df=build(hw,tw,wid,"test"); df_p=df[df["is_post_ps"]==1].reset_index(drop=True)
...
sub.to_csv(SUBS_DIR/"lgbm_final.csv",index=False)
print(f"Saved lgbm_final.csv  NaN={sub.tvt.isna().sum()}")
