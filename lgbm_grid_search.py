...
def per_well_rmse(m, val_ids, FCOLS):
    rmses=[]
    for wid in val_ids:
        try:
...
print("Grid search on regularization...")
grid=[
    ("lam=0.1,ff=0.75,mc=40", 0.1, 0.75, 40),
    ("lam=0.2,ff=0.7,mc=40",  0.2, 0.70, 40),
    ("lam=0.5,ff=0.65,mc=50", 0.5, 0.65, 50),
    ("lam=0.3,ff=0.65,mc=50", 0.3, 0.65, 50),
]
best_row_rmse=15.0821; best_tag="v13_reg"
for tag,lam,ff,mc in grid:
    p={"objective":"regression","metric":"rmse","num_leaves":255,"learning_rate":0.02,
       "feature_fraction":ff,"bagging_fraction":ff,"bagging_freq":5,
       "min_child_samples":mc,"lambda_l1":lam,"lambda_l2":lam,"verbose":-1,"seed":42,"n_jobs":-1}
    ...
    m=lgb.train(p,lgb_tr,num_boost_round=20000,valid_sets=[lgb_vl],
        callbacks=[lgb.early_stopping(500,verbose=False),lgb.log_evaluation(999999)])
    rr=m.best_score["valid_0"]["rmse"]
    pw=per_well_rmse(m,val_ids,FCOLS).mean()
    marker=" ** NEW BEST" if rr<best_row_rmse else ""
    print(f"  {tag}: row={rr:.4f}  per-well={pw:.3f}  iter={m.best_iteration}{marker}")
    if rr<best_row_rmse:
        best_row_rmse=rr; best_tag=tag
        joblib.dump(m,MODELS_DIR/f"lgbm_grid_best.pkl")
        import json; open(MODELS_DIR/"grid_best_params.json","w").write(json.dumps({**p,"best_iter":m.best_iteration}))
print(f"\nBest so far: {best_tag} at {best_row_rmse:.4f}")
