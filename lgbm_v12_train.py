...
def gr_match_tvt(gr_obs, phys_tvt, tw_tvt, tw_gr, search_range=60., step=0.25):
    """Find typewell TVT that best matches observed GR, searching within ±search_range of physics TVT."""
    search = np.arange(-search_range, search_range+step, step)
    out = np.zeros(len(gr_obs), dtype=np.float32)
    out_qual = np.zeros(len(gr_obs), dtype=np.float32)
    for i in range(len(gr_obs)):
        candidates = phys_tvt[i] + search
        tw_grs = np.interp(candidates, tw_tvt, tw_gr, left=tw_gr[0], right=tw_gr[-1])
        diffs = np.abs(tw_grs - gr_obs[i])
        best = np.argmin(diffs)
        out[i] = candidates[best]
        out_qual[i] = diffs[best]
    return out, out_qual
...
def build(hw, tw, well_id, split="train"):
    ...
    # v12: GR matching correction feature
    gr_matched,gr_match_qual=gr_match_tvt(gr,anchored_physics,tw_tvt,tw_gr,search_range=60.)
    gr_match_correction=gr_matched-anchored_physics  # suggested correction from GR matching
    ...
    feat.update({"md":md,"z":z,"dz_dmd":np.gradient(z,md),"dx_dmd":np.gradient(x,md),"dy_dmd":np.gradient(y,md),
        ...
        # v12 new
        "gr_match_correction":gr_match_correction,"gr_match_quality":gr_match_qual})
    if ps>2:
...
print(f"v12: GR-match correction feature. Train={len(train_ids)}, Val={len(val_ids)}")
...
print("Training v12...")
model=lgb.train(params,lgb_tr,num_boost_round=15000,valid_sets=[lgb_vl],
    callbacks=[lgb.early_stopping(500,verbose=False),lgb.log_evaluation(2000)])
joblib.dump(model,MODELS_DIR/"lgbm_v12.pkl")
...
print(f"v12 per-well val RMSE: mean={rmses.mean():.3f}, median={np.median(rmses):.3f}")
...
sub.to_csv(SUBS_DIR/"lgbm_v12.csv",index=False)
print(f"Saved lgbm_v12.csv  NaN={sub.tvt.isna().sum()}")
