"""
LightGBM v18 — 3D physics features
New over v8: adds 3D formation dip (TVT ~ a*X + b*Y + c*Z) as features:
  - phys3d_tvt: per-row 3D physics TVT prediction
  - phys3d_correction: phys3d - phys1d (how much 3D differs from 1D)
  - phys3d_r2_gain: R²(3D) - R²(1D) on pre-PS data (reliability)
  - phys3d_coef_x, phys3d_coef_y: formation dip coefficients in X, Y
  - post_x_range, post_y_range: horizontal extent of post-PS well (ft)
  - post_x_rate, post_y_rate: dX/dMD, dY/dMD in post-PS section
Target: unchanged — correction from 1D anchored physics
"""
import numpy as np
import pandas as pd
import joblib
...
def build(hw, tw, well_id, split="train"):
    ...
    # ── 3D physics features ───────────────────────────────────────────────────
    # Fit TVT ~ a*X + b*Y + c*Z on pre-PS section to capture horizontal formation dip
    pre_x = x[:ps]; pre_y = y[:ps]
    X_anchor = x[max(0, ps - 1)]; Y_anchor = y[max(0, ps - 1)]
    try:
        if ps > 5 and len(pre_z) == len(pre_tvt):
            X3d = np.column_stack([pre_x, pre_y, pre_z])
            r3  = LinearRegression().fit(X3d, pre_tvt)
            a3, b3, c3 = float(r3.coef_[0]), float(r3.coef_[1]), float(r3.coef_[2])
            phys3d = anchor_tvt + a3 * (x - X_anchor) + b3 * (y - Y_anchor) + c3 * (z - Z_anchor)
            p3d_pre = r3.predict(X3d)
            r2_3d = float(1 - np.var(pre_tvt - p3d_pre) / np.var(pre_tvt)) if np.var(pre_tvt) > 0 else 0.
        else:
            raise ValueError("not enough pre-PS rows")
    except Exception:
        phys3d = anchored_physics.copy(); a3 = b3 = 0.; r2_3d = pre_r2

    # Post-PS X, Y extent features
    ...
    # Omit phys3d_tvt (direct TVT prediction – catastrophic for Type A wells, causes early stopping)
    feat.update({
        "phys3d_correction": phys3d - anchored_physics,
        "phys3d_r2_gain":    np.full(n, max(0., r2_3d - pre_r2)),
        "phys3d_coef_x":     np.full(n, a3),
        "phys3d_coef_y":     np.full(n, b3),
        "post_x_range":      np.full(n, post_x_range),
        "post_y_range":      np.full(n, post_y_range),
        "post_x_rate":       np.full(n, post_x_rate),
        "post_y_rate":       np.full(n, post_y_rate),
    })
...
joblib.dump(model, MODELS_DIR / "lgbm_v18b_3dphys.pkl")

# Feature importance
fi = pd.DataFrame({"feature": FCOLS, "importance": model.feature_importance("gain")})
fi = fi.sort_values("importance", ascending=False)
fi.to_csv(MODELS_DIR / "lgbm_v18b_importance.csv", index=False)
...
sub.to_csv(SUBS_DIR / "lgbm_v18b_3dphys.csv", index=False)
print(f"Saved lgbm_v18b_3dphys.csv  NaN={sub.tvt.isna().sum()}")
