"""
LightGBM v22 — Spatial Neighbor 3D Correction Features
=======================================================
...
Key insight:
  For test well 00bbac68 (42% of LB rows), the 3D dip correction
  alpha_x*dx + alpha_y*dy explains 99.5% of physics error variance.
  Nearest training neighbor ba48188d (1200 ft away, val RMSE 51.6 ft)
  has alpha_x=-0.357, alpha_y=-0.227 — a strong regional signal.

Data flow:
  well_dip_coeffs.csv  → neighbor lookup  → per-well alpha constants
  raw CSVs (X, Y cols) → per-row dx, dy   → nbr_3d_corr
  v8 build()           → 46 base features → combined 52-feature matrix
...
# ── LightGBM hyperparameters (from task spec) ──────────────────────────────────
LGBM_PARAMS = {
...
print("=" * 70)
print("LGBM v22 — Spatial Neighbor 3D Correction")
print("=" * 70)
...
fi.to_csv(MODELS_DIR / "lgbm_v22_spatial_importance.csv", index=False)
...
joblib.dump(model_meta, MODELS_DIR / "lgbm_v22_spatial.pkl")
print(f"\n  Saved: models/lgbm_v22_spatial.pkl")
...
for wid in ["ba48188d", "1b1eba53", "389ae58f", "81bf5923"]:
    ...
    if len(match):
        ...
        print(f"  {wid}:  v22={r:.3f} ft  baseline~{b} ft  Δ={delta:+.1f} ft [{tag}]")
...
sub.to_csv(SUBS_DIR / "lgbm_v22_spatial.csv", index=False)
print(f"  Saved: submissions/lgbm_v22_spatial.csv")
