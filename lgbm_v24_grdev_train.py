"""
LightGBM v24 — v8 + GR deviation rolling features
New features: rolling mean/std of gr_dev_physics (the "dip signal"), well-level gr_dev mean/slope.
Hypothesis: the trend of GR deviation encodes formation dip → should fix systematic drift.
...
def build(hw, tw, well_id, split="train"):
    ...
    # GR deviation rolling features — encode formation dip signal
    gr_dev_s = pd.Series(gr_dev)
    for w in [5, 11, 21, 51, 101]:
        dev_roll = gr_dev_s.rolling(w, center=True, min_periods=1)
        feat[f"gr_dev_mean_{w}"] = dev_roll.mean().values
        feat[f"gr_dev_std_{w}"]  = dev_roll.std().fillna(0).values
    ...
    # Well-level GR deviation features: global dip signal constant across all rows
    post_mask_feat = np.arange(n) >= ps
    if post_mask_feat.sum() > 2:
        gr_dev_post = gr_dev[post_mask_feat]
        rows_post   = np.arange(post_mask_feat.sum(), dtype=float)
        gd_mean     = float(gr_dev_post.mean())
        denom = float(np.var(rows_post)) if np.var(rows_post) > 0 else 1.
        gd_slope = float(np.cov(rows_post, gr_dev_post)[0, 1] / denom)
    else:
        gd_mean, gd_slope = 0., 0.
    feat["gr_dev_post_mean"]  = np.full(n, gd_mean)
    feat["gr_dev_post_slope"] = np.full(n, gd_slope)
...
model   = lgb.train(params, lgb_tr, num_boost_round=20000,
                    valid_sets=[lgb_vl],
...
joblib.dump(model, MODELS_DIR / "lgbm_v24_grdev.pkl")

# Feature importance
fi = pd.DataFrame({"feature": FCOLS, "importance": model.feature_importance("gain")})
fi = fi.sort_values("importance", ascending=False)
fi.to_csv(MODELS_DIR / "lgbm_v24_grdev_importance.csv", index=False)
...
sub.to_csv(SUBS_DIR / "lgbm_v24_grdev.csv", index=False)
print(f"Saved lgbm_v24_grdev.csv  NaN={sub.tvt.isna().sum()}")
