"""
LightGBM v19 — v8 + horizontal directional features (post_x_rate, post_y_rate, post_x_range, post_y_range)
Target: correction = true_tvt - anchored_physics
Motivation: captures formation dip in horizontal drilling direction (Type B hard wells)
...
joblib.dump(model, MODELS_DIR / "lgbm_v19_horiz.pkl")

# Feature importance
fi = pd.DataFrame({"feature": FCOLS, "importance": model.feature_importance("gain")})
fi = fi.sort_values("importance", ascending=False)
fi.to_csv(MODELS_DIR / "lgbm_v19_importance.csv", index=False)
...
sub.to_csv(SUBS_DIR / "lgbm_v19_horiz.csv", index=False)
print(f"Saved lgbm_v19_horiz.csv  NaN={sub.tvt.isna().sum()}")
