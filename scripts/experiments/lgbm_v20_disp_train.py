"""
LightGBM v20 — v8 + 4 physically-motivated features for dipping formation wells

New features vs v8:
  x_disp:          cumulative X displacement from PS anchor (X - X_anchor) per row
  y_disp:          cumulative Y displacement from PS anchor (Y - Y_anchor) per row
  gr_dev_cumsum:   running cumulative sum of GR deviation since PS (causal)
  pre_resid_std:   std of pre-PS TVT residuals (stronger signal than pre_r2)

Motivation:
  Type-D dipping wells: correction ≈ α_x * x_disp + α_y * y_disp
  v8 only has dx_dmd/dy_dmd (instantaneous rates) — LGBM cannot integrate them
  x_disp and y_disp provide the cumulative displacement LGBM needs directly
...
def build(hw, tw, well_id, split="train"):
    """Build feature dataframe for one well (v20 features)."""
    n = len(hw)
    ps = get_ps(hw)
    gr = fill_arr(hw["GR"])
    ...
    if len(pre_z) > 5:
        ...
        slope  = -1.0
        pre_r2 = 0.
        pre_resid_std_val = 30.  # assume high noise if no pre-PS data
    ...
    # Well-level horizontal direction features (v19 — rate/range constants)
    ...
    # ── NEW v20: XY cumulative displacement from PS anchor (per row) ─────────
    # correction ≈ α_x * x_disp + α_y * y_disp for dipping formation wells
    # v8 only has dx_dmd/dy_dmd (instantaneous) — LGBM cannot integrate them
    x_anchor = x[max(0, ps - 1)]
    y_anchor = y[max(0, ps - 1)]
    feat["x_disp"] = x - x_anchor   # 0 at PS, grows with horizontal progress
    feat["y_disp"] = y - y_anchor

    # ── NEW v20: Running cumulative GR deviation since PS (causal) ───────────
    # For dipping wells: physics drifts → gr_dev grows → cumsum diverges from 0
    # For flat wells: gr_dev is noisy → cumsum stays near 0
    gr_dev_cumsum = np.zeros(n)
    for i in range(ps, n):
        gr_dev_cumsum[i] = gr_dev_cumsum[i - 1] + gr_dev[i]
    feat["gr_dev_cumsum"] = gr_dev_cumsum

    # ── NEW v20: Pre-PS residual std (raw signal, stronger than pre_r2) ──────
    # NaN-ANCC type wells have high pre_resid_std → suppresses overcorrection
...
joblib.dump(model, MODELS_DIR / "lgbm_v20_disp.pkl")
...
sub.to_csv(SUBS_DIR / "lgbm_v20_disp.csv", index=False)
print(f"Saved lgbm_v20_disp.csv  NaN={sub.tvt.isna().sum()}")
